...
def create_fuqc_plots(df):
    ...
    df_fumee = df[df['Détection modèle'].str.strip() == 'Fumée']
    ...
    part1_buffer = create_graph(incident_counts.iloc[:half_cameras], 
                                f"Nombre d'incidents Fumée  par type de qualification et par caméra (Partie 1) - {date_str}")

    part2_buffer = create_graph(incident_counts.iloc[half_cameras:], 
                                f"Nombre d'incidents Fumée  par type de qualification et par caméra (Partie 2) - {date_str}")
