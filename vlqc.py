...
def create_vlqc_plots(df):
    ...
    part1_buffer = create_graph(incident_counts.iloc[:half_cameras], 
                                f"Nombre d'incidents Véhicule lent par type de qualification et par caméra (Partie 1) - {date_str}")

    part2_buffer = create_graph(incident_counts.iloc[half_cameras:], 
                                f"Nombre d'incidents Véhicule lent par type de qualification et par caméra (Partie 2) - {date_str}")
