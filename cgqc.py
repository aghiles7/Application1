...
def create_cgqc_plots(df):
    # Filtrer les données pour ne garder que les incidents de type "Congestion"
    ...
    part1_buffer = create_graph(incident_counts.iloc[:half_cameras], 
                                f"Nombre d'incidents Congestion par type de qualification et par caméra (Partie 1) - {date_str}")

    part2_buffer = create_graph(incident_counts.iloc[half_cameras:], 
                                f"Nombre d'incidents Congestion par type de qualification et par caméra (Partie 2) - {date_str}")
