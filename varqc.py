...
def create_varqc_plots(df):
    ...
    df_vehicule_arrete = df[df['Détection modèle'].str.strip() == 'Véhicule arrêté']

    # Compter le nombre d'incidents par caméra et par type de qualification
    incident_counts = df_vehicule_arrete.groupby(['Caméra', 'Type de qualification']).size().unstack(fill_value=0)
    ...
    part1_buffer = create_graph(incident_counts.iloc[:half_cameras], 
                                f"Nombre d'incidents Véhicule arrêté par type de qualification et par caméra (Partie 1) - {date_str}")

    part2_buffer = create_graph(incident_counts.iloc[half_cameras:], 
                                f"Nombre d'incidents Véhicule arrêté par type de qualification et par caméra (Partie 2) - {date_str}")
